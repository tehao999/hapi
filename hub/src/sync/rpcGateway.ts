...
export class RpcGateway {
    ...
    async spawnSession(
        machineId: string,
        directory: string,
        agent: 'claude' | 'claude-deepseek' | 'codex' | 'cursor' | 'gemini' | 'opencode' = 'claude',
        model?: string,
        ...
        try {
            const result = await this.machineRpc(
                machineId,
                'spawn-happy-session',
                { type: 'spawn-in-directory', directory, agent, model, modelReasoningEffort, serviceTier, yolo, sessionType, worktreeName, resumeSessionId, effort, permissionMode }
            )
