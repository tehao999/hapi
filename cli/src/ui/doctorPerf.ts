...
export type PerfBackendKind = 'codex' | 'claude' | 'gemini' | 'unknown';
...
function inferBackendKind(session: Pick<ApiSession, 'model'>): PerfBackendKind {
...
function isGeminiCliProcess(processInfo: ProcessInfo): boolean {
    const name = (processInfo.name ?? '').trim();
    const cmd = (processInfo.cmd ?? '').trim();
    return name === 'gemini' || cmd === 'gemini' || cmd.startsWith('gemini ') || /\/gemini(\s|$)/.test(cmd);
}
...
function isBackendProcess(processInfo: ProcessInfo, backendKind: PerfBackendKind): boolean {
    ...
    if (backendKind === 'gemini') {
        return isGeminiCliProcess(processInfo);
    }
    return false;
}
...
function backendMissingWarning(backendKind: PerfBackendKind): string | null {
    ...
    if (backendKind === 'gemini') {
        return 'runner session has no Gemini child';
    }
    return null;
}
